...
public class UserRealmTest extends BaseTest {
	...
	public void testHasPermission() {
		login("classpath:shiro.ini", u1.getUsername(), password);
		org.apache.shiro.subject.Subject subject = subject();
		Session session = subject.getSession(true);
		session.setAttribute("hello", "world");// 会话，类似于web中的session
		login("classpath:shiro.ini", u2.getUsername(), password);
		//切换身份
	...
	/**
	 * 实现线程之间的Subject传播
	 * <V> V execute(Callable<V> callable) throws ExecutionException; void
	 * execute(Runnable runnable); <V> Callable<V> associateWith(Callable<V>
	 * callable); Runnable associateWith(Runnable runnable);
	 */
	@Test
	public void testThreadChangeSubject() {
		
		login("classpath:shiro.ini", u2.getUsername(), password);
		Subject subject = subject();
		Assert.assertFalse(subject.isPermitted("user:create"));
		//将当前线程中的subject过渡到另一个线程中
		subject().execute(new Runnable() {
			
			@Override
			public void run() {
				
				System.out.println(subject == subject());//TRUE
			}
